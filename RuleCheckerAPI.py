...
def RuleCheck(RcObj, DependList, FilePathDict, SelectedItem):
    mdlparser = MDLParserSettings()
    dataLoggerObj = DataLogger.DataLogger(SelectedItem, RcObj)
    Modules = {}
    if len(DependList) > 0:
        progressitems = 100/len(DependList)
    else:
        progressitems = 0
    progressStat = 0
    RcObj.bottonRuleCheck.Enable(False)
    RcObj.staticTextProg.SetLabel("Checking Rules")
    RcObj.gaugeProg.Show()
    RcObj.gaugeProg.SetValue(0)
    for DependLstitem in DependList:
        if CancelRuleCheck == False:
            if DependLstitem in FilePathDict:
                ...
                Modules[DependLstitem] = RuleChecker(dataLoggerObj, resultMD, resultSF, DependLstitem,DependLstitem,SelectedItem)
                Modules[DependLstitem].processAllRulesC()
                # Modules[DependLstitem].processAllRules()
                progressStat+=progressitems
                wx.CallAfter(RcObj.gaugeProg.SetValue, progressStat)
                RcObj.STModuleName.SetLabel(Modules[DependLstitem].ModuleName)
            else:
                RcObj.styledTextCtrl1.AppendText("\n" + '_'* 85 + "\n")
                dispText = 'ERROR    >>>  Dependancy file \"'+DependLstitem+'\" does not exist to process the rule \n'
                RcObj.styledTextCtrl1.AppendText(dispText)
                RcObj.styledTextCtrl1.AppendText('_'* 85 + "\n")

    RcObj.bottonRuleCheck.Enable(True)
    RcObj.STModuleName.SetLabel('')
    if CancelRuleCheck == False:
        RcObj.styledTextCtrl1.AppendText('='* 85 + "\n")
        if dataLoggerObj.RuleCheckFailed == 1:
            dispText = 'RESULT    >>>  FAILED\nRule check details are recorded in >>> '+\
            RcObj.textCtrlFolder.Value +'\\'+ SelectedItem+'_Man.csv\n'
        elif dataLoggerObj.RuleCheckFailed == 2:
            dispText = 'RESULT    >>>  NEEDS MANUAL CHECK\nRule check details are recorded in >>> '+\
            RcObj.textCtrlFolder.Value +'\\'+ SelectedItem+'_Man.csv\n'
        else:
            dispText = 'RESULT    >>>  PASS\nRule check details are recorded in >>> '+\
            RcObj.textCtrlFolder.Value +'\\'+ SelectedItem+'_Man.csv\n'
        RcObj.styledTextCtrl1.AppendText(dispText)
        RcObj.styledTextCtrl1.AppendText("\n" + '='* 85 + "\n")

        RcObj.staticTextProg.SetLabel("Rule Check Done")
        RcObj.gaugeProg.SetValue(100)
        RcObj.dispMsgBox('Rule check is successfully completed. ', 'Success')
        RcObj.gaugeProg.Hide()
        RcObj.staticTextProg.SetLabel("Rule check is successfully completed")
    else:
        RcObj.gaugeProg.Hide()
        RcObj.staticTextProg.SetLabel("Rule Check is Cancelled")
...
class RuleChecker(object):
    ...
    def __init__(self, LoggerObj, MDinlist, SFinlist, ModUndRev,ModelName,selectedModel):
    ...
    def _getCCSettingRuleID(self, configSetting):
        ...
        elif configSetting == 'HISL_0013 A':
            Rule_ID = 'HISL_0013 A'
        elif configSetting == 'HISL_0005 C':
            Rule_ID = 'HISL_0005 C'
        else :
    ...
    #-------------------------------------------------------------------------------------------
    # Decide the rule ID based on the configuration setting key type.
    def _compareConfigSettingKeys(self, CCSetting, configLst, dataDictionaryLst):
        ...
        srchKeys = dataDictionaryLst[CCSetting].keys()
        for srchKeyItem in srchKeys:
            ...
            if srchKeyItem in configLst.keys():
                ...
                self.dataLoggerObj.logCompResult(dataDictionaryLst[CCSetting][srchKeyItem],
                    ...
                    for srchKeysPos in range(0,len(PropDict[0])):
                        if srchKeys.keys()[searchItem] == PropDict[0].keys()[srchKeysPos]:
                            if srchKeys.values()[searchItem] == PropDict[0].values()[srchKeysPos]:
                                self.dataLoggerObj.logCondResult(srchKeys.values()[searchItem], PropDict[0].values()[srchKeysPos], "PASS", " :: ", self.ModuleName, Rule_ID, 'Configuration Settings ',
                                       'Information: Property \"'  '\" is not found in this module')
                            else:
                                self.dataLoggerObj.logCondResult(srchKeys.values()[searchItem], PropDict[0].values()[srchKeysPos], "FAIL", " :: ", self.ModuleName, Rule_ID, 'Configuration Settings ',
                                       'Failure : Property value of \"'+PropDict[0].keys()[srchKeysPos]+'" does not comply with the standards value')
        elif GetStats[0] == -1:
            self.dataLoggerObj.logCondResult("-", "-", "PASS", " :: ", self.ModuleName, Rule_ID, 'Configuration Settings ', 'Information: Property \"'  '\" is not found in this module')

    #-------------------------------------------------------------------------------------------

    def checkConfigSettings_RCAPI(self):
    #check the CC settings stored in the data dicitionary with the parsed data from the
    #MDL file.
        returnCode = 0
                ...
                if (len(srchBlck) > 1):
                    ...
                    if (tGetStats > 0):
                        ...
                        if tPropDict['WSVarName'] in DataDictionary.configReferenceFiles.keys():
                            ...
                            if (len(configLst) > 0):
                                # Loop through all the dictionary CCSettingsList keys
                                for headDictValue in DataDictionary.CCSettingsList.keys():
                                    self._compareConfigSettingKeys(headDictValue, configLst, DataDictionary.CCSettingsList)
                                    
                                # Loop through all the dictionary DataStoreCC keys
                                for headDictValue in DataDictionary.DataStoreCC.keys():
                                    self._compareConfigSettingKeys(headDictValue, configLst, DataDictionary.DataStoreCC)
                        else:
            ...
            for headDictValue in DataDictionary.CCSettingsList.keys():
                ...
                for searchItem in range(0,len(GetStats)):
                    if GetStats[searchItem] == 1:
                        ...
                        self._compareConfigSettingKeys(headDictValue, PropDict[searchItem], DataDictionary.CCSettingsList)
                    ...
                    for index in range(0,len(LinkData)):
                        ...
                        if 'BlockType' in LinkData[index].keys():
                            if LinkData[index]['BlockType'][0] == 'Reference':
                                if headDictValue in LinkData[index]:
                                    if LinkData[index][headDictValue][0] in\
                                        DataDictionary.AllowedReferenceBlocks[headDictValue]:
                                        refTypeAllowed = True
                                    else:
                                        if len(LinkData[index][headDictValue][0]) == 0:
                                            if 'SourceBlock' in LinkData[index].keys():
                                                NullreusableList = ['reusable_lib/deg2rad','reusable_lib/delayed_enable','reusable_lib/rad2deg','reusable_lib/radps2rpm','reusable_lib/rpm2radps']
                                                if LinkData[index]['SourceBlock'][0] in NullreusableList:
                                                    refTypeAllowed = True
    ...
    #-------------------------------------------------------------------------------
    def chckBlckInitParam(self,Rule_ID,srchKeys,ExceptionModule = []):
        '''
          If subSystem have 'ActionPort' then that is conditional Action subSystem.
		  Now take all Outports of it and check for 'InitialOutput'. 
        ...
        if self.ModuleName not in ExceptionModule:
            ...
            while (foundLst == True):
                ...
                outPortBlcksInfo = []			
                CondSubSystemExist = False			
                for Indx in range(0,len(PropDict)):
                    if PropDict[Indx]['BlockType'] =='ActionPort':
   	                    CondSubSystemExist = True
                    if PropDict[Indx]['BlockType'] =='Outport':					
                        outPortBlcksInfo.append(PropDict[Indx])
                if CondSubSystemExist == True:
                    for blckIndex in range(0,len(outPortBlcksInfo)):
                        if 'InitialOutput' in outPortBlcksInfo[blckIndex].keys():
                            if outPortBlcksInfo[blckIndex]['InitialOutput'] == '[]' or len(outPortBlcksInfo[blckIndex]['InitialOutput'])==0:
                                self.dataLoggerObj.logCondResult("should't be null", outPortBlcksInfo[blckIndex]['InitialOutput'], "FAIL", "::", self.ModuleName,
                                     Rule_ID,'InitialOutput of Outport block:\"'+outPortBlcksInfo[blckIndex]['Name']+'" should\'t be null in SUbSystem:\"'+Name+'".')							            
                            else:
                                self.dataLoggerObj.logCondResult("should't be null", outPortBlcksInfo[blckIndex]['InitialOutput'], "PASS", "::", self.ModuleName,
                                     Rule_ID,'InitialOutput of Outport block:\"'+outPortBlcksInfo[blckIndex]['Name']+'" set to \"'+outPortBlcksInfo[blckIndex]['InitialOutput']+'" in SUbSystem:\"'+Name+'".')							            
							
                        else:
                            self.dataLoggerObj.logCondResult("should't be null", "[]", "FAIL", "::", self.ModuleName,
                                 Rule_ID,'InitialOutput of Outport block:\"'+outPortBlcksInfo[blckIndex]['Name']+'" should\'t be null in SUbSystem:\"'+Name+'".')							            
        ...
        if self.ModuleName not in ExceptionModule:
            for propItem in PropChkData.keys():
                ...
                for DefaultBlckItem in DefaultBlkData.keys():
                    ...
                    srchKeys+=UniqueKey
                    srchKeys.append('ActionPort')					
                    inpLst, foundLst, Name = self._getNextList()
                    while (foundLst == True):
                        GetStats, PropDict, rSubLst, CompltLst = self._getSrchDataFromBlck_RCAPI(inpLst['List'], 'Block', srchKeys)
                        #process only if something is found in the list.
                        if Rule_ID == 'MISRA AC SLSF 007 A':
                            CondSubSystemExist = False			
                            for Indx in range(0,len(PropDict)):
                                if PropDict[Indx]['BlockType'] =='ActionPort':
                                    CondSubSystemExist = True
                                        ...
                                        if UniqueKey[0] in PropDict[searchItem].keys():
                                            if PropDict[searchItem][UniqueKey[0]] == DefaultBlckItem:
                                                ...
                                                if propItem in PropDict[searchItem].keys():
                                                    # Compare the parsed parsed property value with the supplied value.
                                                    prptyVal = PropDict[searchItem][propItem] 
                                                    if prptyVal == '[]' or len(prptyVal) == 0:												
                                                        self.dataLoggerObj.logCondResult('intial Output should\'t be null', prptyVal, "FAIL", "::", self.ModuleName,
                                                             Rule_ID, 'Initial Output value of the Block:\"'+BlockName+'" should\'t be set to null in SubSystem:\"'+Name+'"')				  
                                                    else:
                                                        self.dataLoggerObj.logCondResult('intial Output should\'t be null', prptyVal, "PASS", "::", self.ModuleName,
                                                             Rule_ID, 'Initial Output value of the Block:\"'+BlockName+'" set to: \"'+prptyVal+'" in SubSystem:\"'+Name+'"')				  
                                                else:
                                                    dBlockData = DefaultBlkData[DefaultBlckItem]											
                                                    if dBlockData == '[]' or len(dBlockData) == 0:											
                                                        self.dataLoggerObj.logCondResult('intial Output should\'t be null', dBlockData, "FAIL", "::", self.ModuleName,
                                                             Rule_ID, 'Initial Output value of the Block:\"'+BlockName+'" should\'t be set to null in SubSystem:\"'+Name+'"')				  
                                                    else:
                                                        self.dataLoggerObj.logCondResult('intial Output should\'t be null', dBlockData, "PASS", "::", self.ModuleName,
                                                             Rule_ID, 'Initial Output value of the Block:\"'+BlockName+'" set to: \"'+dBlockData+'" in SubSystem:\"'+Name+'"')				  
                        if Rule_ID != 'MISRA AC SLSF 007 A':
                            for searchItem in range(0,len(GetStats)):							
                                if GetStats[searchItem] == 1:
                                    if UniqueKey[0] in PropDict[searchItem].keys():
                ...
                while (foundLst == True):
                    ...
                    for searchItem in range(0,len(GetStats)):
                        if GetStats[searchItem] == 1:
                            ...
                            if len(UniqueKey) < 3:
                                UniqueKey.append('XXX')
                                checkThirdUniqueKey = False
                            else:
                                checkThirdUniqueKey = True
                            
                            if (UniqueKey[0] in PropDict[searchItem].keys()) and ((checkThirdUniqueKey == False) or UniqueKey[2] in PropDict[searchItem].keys()):
                                ...
                                if 'ElseIfExpressions' in PropDict[SrchItem].keys():
                                    if 'Ports' in PropDict[SrchItem].keys():
                                                ...
                                                if PropDict2[LineIndex]['SrcBlock'] == PropDict[SrchItem]['Name']:
                                                    if 'DstPort' in PropDict2[LineIndex].keys():
                                                        if PropDict2[LineIndex]['DstPort'] == 'ifaction':
                                                            ...
                                                            if PropDict2[LineIndex]['SrcBlock'].find("Trig"):
                                                                manualCheckItem = manualCheckItem + 1                                                                
                                        if countItem == noOfOutputPorts:
                                            ...
                                            if manualCheckItem > 0 and manualCheckItem <= noOfOutputPorts:
                                                self.dataLoggerObj.logCondResult("-","-","MANUAL","::",self.ModuleName,Rule_ID,'all outputs of If Block \"'+PropDict[SrchItem]['Name']+'"should be connected to IfActionSubSystem in \"'+Name+'"')
                                            elif countItem == 0:
                                ...
                                if PropDict[SrchItem]['BlockType'] == 'SwitchCase':
                                    if 'Ports' in PropDict[SrchItem].keys():
                                        ...
                                        if countItem == noOfOutputPorts:
                                            ...
                                            if manualCheckItem > 0 and manualCheckItem <= noOfOutputPorts:
                                                self.dataLoggerObj.logCondResult("-","-","MANUAL","::",self.ModuleName,Rule_ID,'all outputs of SwitchCase Block \"'+PropDict[SrchItem]['Name']+'"should be connected to IfActionSubSystem in \"'+Name+'"')
                                            elif countItem == 0:
    ...
    def chkPropValByCond_RCAPI(self, Rule_ID, ListType, PropChkData, CheckListData, ExcludeBlock = [], ExceptionModule = []):
        ...
        if self.ModuleName not in ExceptionModule:
            ...
            self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                 Rule_ID, 'Rule Check is excluded in this Module')
   #-------------------------------------------------------------------------------------------	
    def SignalLabelsShouldName(self,Rule_Id,AllowedBlocks,SrcPrpty,ExceptionModule = []):
        '''
        ...
        if self.ModuleName not in ExceptionModule: 
            inpLst, foundLst, Name = self._getNextList()
            while (foundLst == True):			
                BlckPos = list(self._getItemPosition(inpLst['List'], 'Block'))
                for BlckIndx in BlckPos:
                    if len(BlckIndx) > 0:							
                        BlckData=self._getCmpleteBlck(inpLst['List'], BlckIndx)
                        if len(BlckData) > 0:								
                            BlckType = list(self._getItemPosition(BlckData, 'BlockType'))
                            for typeIndx in BlckType:
                                BlckTypeData=self._getCmpleteBlck(BlckData, typeIndx)
                                if BlckTypeData[0] == 'SubSystem':
                                    self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                                         Rule_Id,'Check Input and output signals of SubSystem:\"'+Name+'" are labelled or not. if not labelled,FAIL this rule.')
                                    break 											
                                elif BlckTypeData[0] in AllowedBlocks[0]:
                                    PropagatedSignalExist = False										
                                    BlckInfo=self._getCmpleteBlck(BlckData, typeIndx[:-1])
                                    PortData = list(self._getItemPosition(BlckInfo, 'Port'))
                                    if len(PortData) >=1:
                                        if PortData[0][1] == 0:
                                            PropagatedSignalExist = True												
                                    if PropagatedSignalExist == True:
                                        self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                             Rule_Id,'Output signal name of Block:\"'+BlckInfo[0][1]+'" is labelled in System or SubSystem:\"'+Name+'"')
                                    else:
                                        self.checkForNonPropSignals(self.ModuleName,Rule_Id,0,inpLst['List'],BlckInfo[0][1],'SrcBlock',Name,'Match',BlckTypeData[0])			
                                elif BlckTypeData[0] in AllowedBlocks[1]:					
                                    numberOfOutports = 0										
                                    BlckInfo=self._getCmpleteBlck(BlckData, typeIndx[:-1])
                                    #get number of output ports
                                    PortsList = list(self._getItemPosition(BlckInfo, 'Ports')) 											
                                    #print PortsList											
                                    #get Ports block						
                                    if len(PortsList) > 0: 											
                                        PortsInfo=self._getCmpleteBlck(BlckInfo, PortsList[0])
                                        if len(PortsInfo[0]) != 0: 												
                                            numberOfOutports=PortsInfo[0][1]
                                    if numberOfOutports != 0:
                                        PortData = list(self._getItemPosition(BlckInfo, 'Port'))
                                        numOfPropagatedSignals = 0												
                                        for PortIndx in PortData:
                                            if PortIndx[1]==0 and len(PortIndx) == 2:
                                                numOfPropagatedSignals=numOfPropagatedSignals+1
                                        if numOfPropagatedSignals >= numberOfOutports:
                                            self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                                Rule_Id,'Output signal names of Block:\"'+BlckInfo[0][1]+'" is labelled in System or SubSystem:\"'+Name+'"')
                                        else:
                                            self.checkForNonPropSignals(self.ModuleName,Rule_Id,numberOfOutports,inpLst['List'],BlckInfo[0][1],'SrcBlock',Name,'Match2',BlckTypeData[0])			
                                elif BlckTypeData[0] in AllowedBlocks[2]:
                                    numberOfOutports = 0										
                                    BlckInfo=self._getCmpleteBlck(BlckData, typeIndx[:-1])
                                    #get number of output ports
                                    PortsList = list(self._getItemPosition(BlckInfo, 'Ports')) 											
                                    ...
                                    if len(PortsList) > 0: 											
                                        PortsInfo=self._getCmpleteBlck(BlckInfo, PortsList[0])
                                        if len(PortsInfo[0]) != 0:
                                            if len(PortsInfo[0]) == 2:
                                                numberOfOutports=PortsInfo[0][1]
                                            elif len(PortsInfo[0]) == 5:
                                                numberOfOutports=PortsInfo[0][1]+PortsInfo[0][4]											
                                    if numberOfOutports != 0:
                                        PortData = list(self._getItemPosition(BlckInfo, 'Port'))
                                        numOfPropagatedSignals = 0												
                                        for PortIndx in PortData:
   ...
   #-------------------------------------------------------------------------------------------
    ...
    def checkNoUnconnectedBlocks_RCAPI(self,Rule_Id,BlockList,AllowedBlockList,ExceptionModule = ['reusable_lib']):
        ...
        if self.ModuleName not in ExceptionModule:
            ...
            while (foundLst == True):
                ...
                GetStatsS1, PropDictS1, rSubLstS1, CompltLst = self._getSrchDataFromBlck_RCAPI(inpLst['List'], 'Block', ['BlockType','Name','Ports','VariantObject'])
                GetStatsS2, PropDictS2, rSubLstS2, CompltLst = self._getSrchDataFromBlck_RCAPI(inpLst['List'], 'Line', LineDataSrchKeys)
                for  BlockIndex in range(0,len(PropDictS1)):
                    variantBlocks = False
                    if 'VariantObject' in PropDictS1[BlockIndex].keys():
                        variantBlocks = True
                    #for either ['Inport','From','Ground','Constant'] Blocks
                    if PropDictS1[BlockIndex]['BlockType'] in AllowedBlockList[0]:
                        if 'Name' in PropDictS1[BlockIndex].keys() and variantBlocks == True:
                        ...
                        if PropDictS1[BlockIndex]['BlockType'] in AllowedBlockList[1]:
                            ...
                            if  PropDictS1[BlockIndex]['BlockType'] in  AllowedBlockList[2]:
                                if 'Name' in PropDictS1[BlockIndex].keys():
                                    NameOfBlock = PropDictS1[BlockIndex]['Name']
                                if 'Ports' in PropDictS1[BlockIndex].keys() and variantBlocks == True:
    ...
    def GetBlockPortData(self,SystemData,SrchBlck,SrchAttr):
        '''
           Get 'SrchAttr' attrib of 'SrchBlck' from 'SystemData'
        ''' 
        GetStats = 0
        PropDict = []
        rSubLst = []
        BlckPos = list(self._getItemPosition(SystemData, 'Block'))		
        for BlckIndx in BlckPos:
            if len(BlckIndx) > 0:
                BlckData=self._getCmpleteBlck(SystemData, BlckIndx)		
                for Blckitem in BlckData:
                    if Blckitem[0] == 'Name':
                        if Blckitem[1] == SrchBlck: 
                            #fetch Port block property 'ShowPropagatedSignals',if Port exists in Block data.
                            portPos = list(self._getItemPosition(BlckData, 'Port'))
                            for portIndx in portPos:
                                portData=self._getCmpleteBlck(BlckData, portIndx)
                                tGetStats, tPropDict, trSubLst = self._getGenericBlckPrpty(portData, [SrchAttr])
                                if tGetStats > 0:
                                    GetStats = 1
                                    PropDict.append(tPropDict)
                                    rSubLst.append(trSubLst)
        return GetStats, PropDict, rSubLst
    #--------------------------------------------------------------------------------------    
    def checkTagNameAndSignalOrBusMustMatch_RCAPI(self, Rule_ID, PropChkData, UniqueKey, CheckType,AllowedBlocks,ExceptionModule = []):
        '''
		Misra 18B:
		          first take the values of [gotoTag,Name] properties from GOTO and FROM blocks.similarly extract [srcBlock,Name]
                ...
                if CheckType == 'Exact':
                                ...
                                if PropDictS1[searchItem][UniqueKey[0]] == PropChkData['SourceBlockType']:
                                    ...
                                    if PropChkData['SourceProp'] in PropDictS1[searchItem].keys():
                                        ...
                                        if 'Name' in PropDictS1[searchItem]:
                                            ...
                                            #ListForFrom holds the values of Block type From of [GotoTag property value,Name property value]
                    ...
                    if len(ListForFrom)>0 and len(ListForGoto)>0:
                        ...
                        for SrchItem in range(0,len(ListForFrom)):
                                ...
                                if ResultType == 0:
                                    self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                         Rule_ID,'No Signal from FROM Block \"'+ListForFrom[SrchItem][1]+'\" of SourceBlock type '+PropDictS2[SrchItem]['SrcBlock'])
                                else:
                                    if ResultType == 1:
                                        if 'Name' in PropDictS2[matchCase2].keys():
                                            #compare signal or bus name and FROM block name.
                                            labelName = PropDictS2[matchCase2]['Name']
                                            if labelName ==  ListForFrom[matchCase1][0]:
                                                self.dataLoggerObj.logCondResult(labelName, ListForFrom[matchCase1][0], "PASS", "::", self.ModuleName,
                                                    Rule_ID, 'FROM Block \"'+ListForFrom[SrchItem][1]+'\" with TagName: \"'+ListForFrom[matchCase1][0]+'\" matches the corresponding Signal Name\"'+PropDictS2[matchCase2]['Name']+'\" in \"'+Name+'\".')
                                            else:
                                                self.dataLoggerObj.logCondResult(labelName, ListForFrom[matchCase1][0], "FAIL", "::", self.ModuleName,
                                                    Rule_ID, 'FROM Block \"'+ListForFrom[SrchItem][1]+'\" with TagName: \"'+ListForFrom[matchCase1][0]+'\" does not match with the corresponding Signal Name \"'+PropDictS2[matchCase2]['Name']+'\" in \"'+Name+'\".')
                                        else:
                                            # fetch the Port data from the From block
                                            pGetStats, pPropDict, prSubLst = self.GetBlockPortData(inpLst['List'], ListForFrom[SrchItem][1], 'PropagatedSignals')
                                            if pGetStats > 0:
                                                if pPropDict[0]['PropagatedSignals'] == ListForFrom[matchCase1][0]:
                                                    self.dataLoggerObj.logCondResult(pPropDict[0]['PropagatedSignals'], ListForFrom[matchCase1][0], "PASS", "::", self.ModuleName,
                                                        Rule_ID, 'FROM Block \"'+ListForFrom[SrchItem][1]+'\" with TagName: \"'+ListForFrom[matchCase1][0]+'\" matches the corresponding Signal Name \"'+pPropDict[0]['PropagatedSignals']+'\" in \"'+Name+'\".')
                                                else:
                                                    self.dataLoggerObj.logCondResult(pPropDict[0]['PropagatedSignals'], ListForFrom[matchCase1][0], "FAIL", "::", self.ModuleName,
                                                        Rule_ID, 'FROM Block \"'+ListForFrom[SrchItem][1]+'\" with TagName: \"'+ListForFrom[matchCase1][0]+'\" does not match with the corresponding Signal Name \"'+pPropDict[0]['PropagatedSignals']+'\" in \"'+Name+'\".')
                                            else:
                                                self.dataLoggerObj.logCondResult('-', '-', "FAIL", "::", self.ModuleName,
                                                    Rule_ID, 'The signal originating from FROM Block \"'+ListForFrom[SrchItem][1]+'\" with TagName: \"'+ListForFrom[matchCase1][0]+'\" is not labelled in \"'+Name+'\".')
                        ...
                        for SrchItem in range(0,len(ListForGoto)):
                                ...
                                if ResultType == 0:
                                    ...
                                    if ResultType == 1:
                                        if 'Name' in PropDictS2[matchCase2].keys():
                                            labelName = PropDictS2[matchCase2]['Name']
                                            if labelName ==  ListForGoto[matchCase1][0]:
                                                self.dataLoggerObj.logCondResult(labelName, ListForGoto[matchCase1][0], "PASS", "::", self.ModuleName,
                                                    Rule_ID, 'GOTO Block \"'+ListForGoto[matchCase1][1]+'\" with TagName: \"'+ListForGoto[matchCase1][0]+'\" matches the corresponding Signal Name \"'+PropDictS2[matchCase2]['Name']+'\" in \"'+Name+'\".')
                                            else:
                                                self.dataLoggerObj.logCondResult(labelName, ListForGoto[matchCase1][0], "FAIL", "::", self.ModuleName,
                                                    Rule_ID, 'GOTO Block \"'+ListForGoto[matchCase1][1]+'\" with TagName: \"'+ListForGoto[matchCase1][0]+'\" does not match with the corresponding Signal Name \"'+PropDictS2[matchCase2]['Name']+'\" in \"'+Name+'\".')
                                        else:
                                            gotoScrBlk = PropDictS2[SrchItem2]['SrcBlock']
                                            # fetch the Port data from the GOTO block
                                            inputData = inpLst['List']
                                            blkPortDataFound = 0
                                            currSubsysCntr = self.subsystemCntr - 1
                                            while blkPortDataFound == 0:
                                                pGetStats, pPropDict, prSubLst = self.GetBlockPortData(inputData, gotoScrBlk, 'PropagatedSignals')
                                                if pGetStats > 0:
                                                    blkPortDataFound = 1
                                                else:
                                                    if(currSubsysCntr > 1):
                                                        currSubsysCntr = currSubsysCntr - 1
                                                        inputData = self.SubSysData[currSubsysCntr]['List']
                                                    else:
                                                        break
                                            ...
                                            if pGetStats > 0:
                                                sigFound = 0
                                                propsigFound = 0
                                                for listIndx in range (0,len(pPropDict)):
                                                    if 'PropagatedSignals' in pPropDict[listIndx].keys():
                                                        propsigFound = 1
                                                        if pPropDict[listIndx]['PropagatedSignals'] == ListForGoto[matchCase1][0]:														    
                                                            self.dataLoggerObj.logCondResult(pPropDict[listIndx]['PropagatedSignals'], ListForGoto[matchCase1][0], "PASS", "::", self.ModuleName,
                                                                Rule_ID, 'GOTO Block \"'+ListForGoto[matchCase1][1]+'\" with TagName: \"'+ListForGoto[matchCase1][0]+'\" matches the corresponding Signal Name \"'+pPropDict[listIndx]['PropagatedSignals']+'\" in \"'+Name+'\".')
                                                            sigFound = 1
                                                            break
                                                if sigFound == 0:
                                                            self.dataLoggerObj.logCondResult(pPropDict[listIndx]['PropagatedSignals'], ListForGoto[matchCase1][0], "FAIL", "::", self.ModuleName,
                                                                Rule_ID, 'GOTO Block \"'+ListForGoto[matchCase1][1]+'\" with TagName: \"'+ListForGoto[matchCase1][0]+'\" does not match with the corresponding Signal Name \"'+pPropDict[listIndx]['PropagatedSignals']+'\" in \"'+Name+'\".')
                                                if propsigFound == 0:
                                                    self.dataLoggerObj.logCondResult('-','-', "FAIL", "::", self.ModuleName,
                                                        Rule_ID, 'The signal corresponding GOTO Block \"'+ListForGoto[matchCase1][1]+'\" with TagName: \"'+ListForGoto[matchCase1][0]+'\" is not labelled in \"'+Name+'\".')
                                            else:
                                                self.dataLoggerObj.logCondResult('-','-', "FAIL", "::", self.ModuleName,
                                                    Rule_ID, 'The signal corresponding GOTO Block \"'+ListForGoto[matchCase1][1]+'\" with TagName: \"'+ListForGoto[matchCase1][0]+'\" is not labelled in \"'+Name+'\".')
                else:
                        ...
                        for BlckIndx in BlckPos:
                            if len(BlckIndx) > 0:							
                                ...
                                if len(BlckData) > 0:								
                                    ...
                                    for typeIndx in BlckType:
                                        BlckTypeData=self._getCmpleteBlck(BlckData, typeIndx)
                                        #print BlckTypeData,typeIndx										
                                        if BlckTypeData[0] == 'SubSystem':
                                            numberOfOutports = 0										
                                            #get number of output ports
                                            PortsList = list(self._getItemPosition(BlckData, 'Ports')) 											

                                            # #get Ports block						
                                            if len(PortsList) > 0:
											    #check for second element in the Ports list
                                                if PortsList[0][1] == 0:  											
                                                    PortsInfo=self._getCmpleteBlck(BlckData, PortsList[0])
                                                    if len(PortsInfo[0])!=0: 													
                                                        numberOfOutports=PortsInfo[0][1]
                                            if numberOfOutports != 0:
                                                PortData = list(self._getItemPosition(BlckData, 'Port'))
                                                # count the number of ports 												
                                                numOfPropagatedSignals = 0												
                                                for PortIndex in PortData:
                                                    if PortIndex[1]==0 and len(PortIndex) == 2:
                                                        numOfPropagatedSignals=numOfPropagatedSignals+1
                                                if numOfPropagatedSignals == numberOfOutports:											
                                                    self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                                         Rule_ID,'All signals or Bus originating from Block:\"'+BlckData[1][1]+'"is labelled with propagated signal in System or SubSystem:\"'+Name+'".')
                                                else:
                                                    self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                                         Rule_ID,'Signal or Bus originating from Block:\"'+BlckData[1][1]+'"Must be labelled with propagated signal in System or SubSystem:\"'+Name+'".')												
												
                                            break 											
                                        elif BlckTypeData[0] in ['From']:					
                                            PropagatedSignalExist = False										
                                            BlckInfo=self._getCmpleteBlck(BlckData, typeIndx[:-1])
                                            PortData = list(self._getItemPosition(BlckInfo, 'Port'))
                                            if len(PortData) >=1:
                                            ...
                                            if PropagatedSignalExist == True:
                                                self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                                     Rule_ID,'Signal or Bus originating from Block:\"'+BlckInfo[0][1]+'"is labelled with propagated signal in System or SubSystem:\"'+Name+'".')
                                            else:
                                                self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                                     Rule_ID,'Signal or Bus originating from Block:\"'+BlckInfo[0][1]+'"Must be labelled with propagated signal in System or SubSystem:\"'+Name+'".')
           												
                                        elif BlckTypeData[0] in ['Demux','Selector']:					
                                            numberOfOutports = 0										
                                            BlckInfo=self._getCmpleteBlck(BlckData, typeIndx[:-1])
                                            #get number of output ports
                                            PortsList = list(self._getItemPosition(BlckInfo, 'Ports')) 											
                                            ...
                                            if numberOfOutports != 0:
                                                ...
                                                if numOfPropagatedSignals == numberOfOutports:
                                                    self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                                         Rule_ID,'All signals or Bus originating from Block:\"'+BlckInfo[0][1]+'"is labelled with propagated signal in System or SubSystem:\"'+Name+'".')
                                                else:
                                                    self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                                         Rule_ID,'Signal or Bus originating from Block:\"'+BlckInfo[0][1]+'"Must be labelled with propagated signal in System or SubSystem:\"'+Name+'".')												
                        ...
                        for BlkIndex in range(0,len(PropDictS1)):
                            if 'BlockType' in PropDictS1[BlkIndex].keys():
                                if PropDictS1[BlkIndex]['BlockType'] == 'Switch':
                                    self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                                         Rule_ID, 'Check the Control Input of Switch Block:\"'+PropDictS1[BlkIndex]['Name']+'" in System or SubSystem:\"'+Name+'".If it is Boolean,then switch block criteria must set to "u~=0".If it is not boolean,threshold must set to 0.5.')
                    else:
                        if CheckType == 'Unique':
                                ...
                                if PropDictS1[searchPos]['BlockType'] == 'Merge':
                                    ...
                                    if NameExist == True:
                                        ...
                                        if rcount == noOfinIputPorts:
                                            ...
                                            self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                                                Rule_ID, 'MANUAL CHECK: Check the output signal of merge block at: \"'+path+'".If it is propagated signal, then check whether output and all input signals are matching or not.If not(not a propagated signal or not matching input and output signals), fail this rule.')
                                ...
                                for BlockIndex in range(0,len(PropDictS1)):
                                    ...
                                    if UniqueKey[0] in PropDictS1[BlockIndex].keys():
                                        ...
                                        if PropDictS1[BlockIndex][UniqueKey[0]] in AllowedBlocks:
                                            blocksExist = True                                        
                                            self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                                                 Rule_ID, 'check whether all Input signals of blockType:\"'+PropDictS1[BlockIndex][UniqueKey[0]]+'" with Name:\"'+PropDictS1[BlockIndex]['Name']+'" are labelled or not,in System or SubSystem:\"'+Name+'" .If not, Fail this rule.')
                                if blocksExist == False:
                                    self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                         Rule_ID,'"GOTO,MUX,SubSystem,Chart" blocks are not exist in System or SuubSystem:\"'+Name+'"')
                        ...
                        if PropChkData['SourceProp'] in PropDictS1[searchItem].keys():
                            ...
                            if 'SrcBlock' in Srcblk[searchItem]:							
                                if SourceProp not in SourcePropList:
                                    ...
                                    self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                                         Rule_ID, 'check whether signal Name:\"'+SourceProp+'\" is unique or not in System or subSystem:\"'+Name+'".If not unique, then FAIL this rule.')
                                SourcePropList.append(SourceProp)
                inpLst, foundLst, Name = self._getNextList()
        else:
            self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
            ...
            for propItem in PropChkData.keys():
                ...
                for searchItem in range(0,len(GetStats)):
                    if GetStats[searchItem] == 1:
                        ...
                        self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                             Rule_ID, 'List \"'+ListType+'\" is not found in the model')
        else:
            self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                 Rule_ID, 'Rule Check is excluded in this Module')
    ...
    def checkEMLBlockdonotExist_RCAPI(self, Rule_ID,resultType,SFInfo,ExceptionModule = []):
        '''
		Misra 005: Retrieve all Embedded MATLAB Block Names from System properties and store it
        ...
        if self.ModuleName not in ExceptionModule:
            if resultType == 'NotExist':
                        ...
                        if inpLst['List'][PrptyPos[0][0]][1] == 'Stateflow':
                            ...
                            if len(PrptyPos) > 0:
                                if inpLst['List'][PrptyPos[0][0]][1] == 'Embedded MATLAB block':
                                    ...
                                    if resultType == 'NotExist':
                                        self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                             Rule_ID,'Embedded MATLAB Block: \"'+nameOfEMLBlck+'" is not a allowed Block in Simulink.')
                ...
                if len(EMLNames)==0:
                    self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                         Rule_ID,'Embedded MATLAB Block is not Exist')
            else:
                if resultType == 'Exist':
                        ...
                        for chartIndex in range(0,len(chratNames)):
                            ...
                            for EMLIndex in range(0,len(EMLInfo)):
                                if chratNames[chartIndex]['id'] == EMLInfo[EMLIndex]['chartID']:
                                    resultFound = 1
                                    self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                         Rule_ID,'Embedded Matlab Block:\"'+EMLInfo[EMLIndex]['lableString']+'" is not allowed in stateFlow :\"'+chratNames[chartIndex]['name']+'"')
                            if resultFound == 0:
                                self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                     Rule_ID,'Embedded Matlab Block is not Exist in stateFlow :\"'+chratNames[chartIndex]['name']+'"')
                    else:
    ...
    def checkSrcDataByLink_RCAPI(self, Rule_ID, SrcInput, DstInput, CheckList):
        ...
        while (foundLst == True):
            ...
            inpLst, foundLst, Name = self._getNextList()
    #-------------------------------------------------------------------------------------------
    def GetBlockData(self,SystemData,SrchBlck):
        '''
           Get only block information which doesn't have "Ports" sub-Block with "PropagatedSignals"
        ''' 		
        BlckPos = list(self._getItemPosition(SystemData, 'Block'))		
        BlckNames = []		
        for BlckIndx in BlckPos:
            if len(BlckIndx) > 1:		
                BlckData=self._getCmpleteBlck(SystemData, BlckIndx)		
                #fetch Port block property 'ShowPropagatedSignals',if Port exists in Block data.
                BlckPos2 = list(self._getItemPosition(BlckData, 'ShowPropagatedSignals'))						
                if len(BlckPos2) == 0: 				
                    GetStats, PropDict, rSubLst = self._getGenericBlckPrpty(BlckData, ['BlockType','Name'])                    				
                    if PropDict['BlockType'] == SrchBlck:					
                        BlckNames.append(PropDict)
        return BlckNames					
        #print SystemData,BlckPos,'**********'		
    #-------------------------------------------------------------------------------------------
    def InportLabelString_RCAPI(self,RULE_ID,PrpDict,LstType,MatchType,ExceptionModule = []):
        '''
           Checking label Names of Inport blocks. 
        '''
        if self.ModuleName not in ExceptionModule:		
            inpLst, foundLst, Name = self._getNextList()
            while (foundLst == True):

                #Get non-propagated Block names
                NonPrapgatedBlcks=self.GetBlockData(inpLst['List'],PrpDict['BlockType']) 			
                if len(NonPrapgatedBlcks) > 0:
                    #Get lines data with [Name,SrcBlock]			
                    LineDataInSystem  = self._getSrchDataFromSubBlck_RCAPI(inpLst['List'], 'Line', ['Name','SrcBlock'])
                    #Check whether non-propagated signals are named or not
                    for BlckIndx in range(0,len(NonPrapgatedBlcks)):
                        NameExist = False
                        for LineIndx in range(0,len(LineDataInSystem)):
                            if 'SrcBlock' in LineDataInSystem[LineIndx].keys():					
                                if NonPrapgatedBlcks[BlckIndx]['Name'] == LineDataInSystem[LineIndx]['SrcBlock'][0]:
                                    NameExist=True
                                    if 'Name' in LineDataInSystem[LineIndx].keys():
                                        self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                             RULE_ID,'\"'+PrpDict['BlockType']+'" signal of Block:\"'+NonPrapgatedBlcks[BlckIndx]['Name']+'"is labelled in:\"'+Name+'"')
                                    else:
                                        self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                             RULE_ID,'\"'+PrpDict['BlockType']+'" signal of Block:\"'+NonPrapgatedBlcks[BlckIndx]['Name']+'"is not labelled in:\"'+Name+'"')
                        if NameExist == False:
                            self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                 RULE_ID,'\"'+PrpDict['BlockType']+'" signal of Block:\"'+NonPrapgatedBlcks[BlckIndx]['Name']+'"is not labelled in:\"'+Name+'"')
                inpLst, foundLst, Name= self._getNextList()
            ...
            self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                 RULE_ID, 'Rule Check is excluded in this Module')			
    #-------------------------------------------------------------------------------------------
    def OutportLabelString_RCAPI(self,RULE_ID,PrpDict,LstType,MatchType,ExceptionModule = []):
        '''
           Checking label Names of  outport blocks. 
        '''
        if self.ModuleName not in ExceptionModule:		
            inpLst, foundLst, Name = self._getNextList()
            while (foundLst == True):
                OutportBlck = []			
                #Fecth Outport Blocks
                GetStats, PropDict, rSubLst, CompltLst = self._getSrchDataFromBlck_RCAPI(inpLst['List'], 'Block', ['BlockType','Name']) 				
                for BlckIndx in range(0,len(PropDict)):
                    if PropDict[BlckIndx]['BlockType'] == PrpDict['BlockType']:                      				
                        OutportBlck.append(PropDict[BlckIndx])

                # Fetch Line data with specified properties  
                LGetStats, LPropDict, LrSubLst, LCompltLst = self._getSrchDataFromBlck_RCAPI(inpLst['List'], 'Line', ['SrcBlock','DstBlock','Name']) 				              
                # check line data whether Signal is labelled or not
                for BlkIndx in range(0,len(OutportBlck)):
                    NameExist = False				
                    for LineIndx in range(0,len(LPropDict)):
                        if 'DstBlock' in LPropDict[LineIndx].keys():
                            if OutportBlck[BlkIndx]['Name'] == LPropDict[LineIndx]['DstBlock']:
                                NameExist = True							
                                if 'Name' in LPropDict[LineIndx].keys():
                                    self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                        RULE_ID,'Input signal of OutPort Block:\"'+OutportBlck[BlkIndx]['Name']+'"is labelled in:\"'+Name+'"')                                    								
                                else:
                                    self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                                        RULE_ID,'MANUAL CHECK :check the Input signal of OutPort Block:\"'+OutportBlck[BlkIndx]['Name']+'" in:\"'+Name+'".If it is propagated signal then PASS this rule otherwise FAIL it')                                    								
                    if NameExist == False:								
                        self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                        RULE_ID,'Input signal of OutPort Block:\"'+OutportBlck[BlkIndx]['Name']+'"must be labelled in:\"'+Name+'"')               	    		 	
                inpLst, foundLst, Name= self._getNextList()
        else:
            self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                 RULE_ID, 'Rule Check is excluded in this Module')			
    ...
    # def NonEmptyStringForInOutport_RCAPI(self, RULE_ID, PropDic, ListType, CheckType, ExceptionModule = []):
        ...
        # if self.ModuleName not in ExceptionModule:
            # for IndexOfSystem in range(0,len(SystemInfoList)):
                ...
                # #Get Block which doesn't have 'Port' sub-block
                # #BlockDataInSystem = self.GetBlockData(SystemInfoList,srchKeys) 	
                            ...
                            # for SearchItem in range(0,len(NamesList)):
                                # if  SrcBlockLineList[SearchItemForList][PropDic['SourceProp']][0] ==  NamesList[SearchItem]:
                                    # if CheckType == 'Exist':
                                        ...
                                        # else:
                                            # self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                                                 # RULE_ID, 'Check the input and output signals of block:\"'+SrcBlockLineList[SearchItemForList][PropDic['SourceProp']][0]+'\" are labelled or not.If not labelled, then FAIL it.')
        # else:
            # self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                 # Rule_ID, 'Rule Check is excluded in this Module')
    ...
    def ManualCheck_RCAPI(self,Rule_ID,resultInfo,ExceptionModule = []):
        ...
        if self.ModuleName not in ExceptionModule:
            ...
            self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                 Rule_ID, 'Rule Check is excluded in this Module')
    #-------------------------------------------------------------------------------------------	
    def InputSignalshouldLabel_RCAPI(self,Rule_ID,checkType,BlockType,ExceptionModule = []):
        '''
            Extract the 'blockType' from every block,If it maches with 
            received block type, then check for Port block(propagated signal) in same block            
            If port block not exist then check the 'Name' property(non-propagated) in Line block.            
        ...
        if self.ModuleName not in ExceptionModule:
                    ...
                    if len(BlckIndx) > 0:
                        ...
                        if len(BlckData) > 0:								
                            ...
                            for typeIndx in BlckType:
                                ...
                                if BlckTypeData[0] == 'SubSystem':
                                    break
                                elif BlckTypeData[0] in BlockType['BlockType']:
                                    if checkType == 'NameExist':                                
                                        numberOfInputs = 0                                
                                        BlckInfo=self._getCmpleteBlck(BlckData, typeIndx[:-1])
                                        PortsList = list(self._getItemPosition(BlckInfo, 'Ports')) 											
                                        #get Ports block						
                                        if len(PortsList) > 0: 											
                                            PortsInfo=self._getCmpleteBlck(BlckInfo, PortsList[0])
                                            numberOfInputs=PortsInfo[0][0]
                                        PropSignalData = list(self._getItemPosition(BlckInfo, 'PropagatedSignals'))
                                        busCreatorBlckName = BlckInfo[0][1]  
                                        #If Ports block not exist, then signal in Line property                                     
                                        if len(PropSignalData) == 0:
                                            self.checkForNonPropSignals(self.ModuleName,Rule_ID,numberOfInputs,inpLst['List'],busCreatorBlckName,'DstBlock',Name,checkType,'BusCreator')
                                        else:
                                            propSignalPos = PropSignalData[0]
                                            inputSignals=self._getCmpleteBlck(BlckInfo, propSignalPos)
                                            InputSignalsList=inputSignals[0].split(',')                                          
                                            if len(InputSignalsList) == numberOfInputs:
                                                self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                                                     Rule_ID,'BusCreator block:\"'+busCreatorBlckName+'" input signals are named in System or SubSystem:\"'+Name+'".')                                        
                                            else:
                                                self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", self.ModuleName,
                                                     Rule_ID,'BusCreator block:\"'+busCreatorBlckName+'"must not contain unnamed input signals in System or SubSystem:\"'+Name+'".')                                        
                                    elif checkType == 'Exact':
                                        BlckInfo=self._getCmpleteBlck(BlckData, typeIndx[:-1])
                                        PropSignalData = list(self._getItemPosition(BlckInfo, 'PropagatedSignals'))
                                        NameOftheBlock = BlckInfo[0][1]                                        
                                        if len(PropSignalData) == 0:
                                            if BlckTypeData[0] == 'Outport':                                         
                                                self.checkForNonPropSignals(self.ModuleName,Rule_ID,1,inpLst['List'],NameOftheBlock,'DstBlock',Name,checkType,BlckTypeData[0])
                                            else:
                                                self.checkForNonPropSignals(self.ModuleName,Rule_ID,1,inpLst['List'],NameOftheBlock,'SrcBlock',Name,checkType,BlckTypeData[0])
                                        else:
                                            propSignalPos = PropSignalData[0]
                                            inputSignalName=self._getCmpleteBlck(BlckInfo, propSignalPos)                                          
                                            if inputSignalName[0] == NameOftheBlock:
                                                self.dataLoggerObj.logCondResult(NameOftheBlock,inputSignalName[0],"PASS", "::", self.ModuleName,
                                                     Rule_ID,'Signal Name and Port names are matched in BlockType:\"'+BlockType['BlockType']+'" of Name:\"'+NameOftheBlock+'" in System or SubSystem:\"'+Name+'".')                                        
                                            else:
                                                self.dataLoggerObj.logCondResult(NameOftheBlock,inputSignalName[0], "FAIL", "::", self.ModuleName,
                                                     Rule_ID,'Signal Name and Port names are mis-matched in BlockType:\"'+BlockType['BlockType']+'" of Name:\"'+NameOftheBlock+'" in System or SubSystem:\"'+Name+'".')
                inpLst, foundLst, Name = self._getNextList()
            ...
            self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", self.ModuleName,
                 Rule_ID, 'Rule Check is excluded in this Module')
    #-------------------------------------------------------------------------------------------	
    def checkForNonPropSignals(self,MdlName,Rule_ID,noOfPortSignals,inputList,BlckName,proprty,Name,chekType,BlockType):
        '''
            This function checks the 'Name' property in 'Line' block.        
        '''
        GetStats, PropDict, rSubLst, CompltLst = self._getSrchDataFromBlck_RCAPI(inputList, 'Line',['Name','DstBlock','SrcBlock'])        
        numOfSignalsinLine=0
        NameOfSignal = "<unknown>"        
        for LIndex in range(0,len(PropDict)):
            if proprty in PropDict[LIndex].keys():
                if PropDict[LIndex][proprty] == BlckName:
                    if 'Name' in PropDict[LIndex].keys():
                        numOfSignalsinLine = numOfSignalsinLine+1
                        NameOfSignal=PropDict[LIndex]['Name']
        if chekType == 'NameExist':                         
            if numOfSignalsinLine == noOfPortSignals:
                self.dataLoggerObj.logCondResult("-", "-", "PASS", "::", MdlName,
                     Rule_ID,'BusCreator block:\"'+BlckName+'" input signals are named in System or SubSystem:\"'+Name+'".')                                        
            else:
                self.dataLoggerObj.logCondResult("-", "-", "FAIL", "::", MdlName,
                     Rule_ID,'BusCreator block:\"'+BlckName+'"must not contain unnamed input signals in System or SubSystem:\"'+Name+'".')
        elif chekType == 'Exact':
            if NameOfSignal == BlckName:
                self.dataLoggerObj.logCondResult(BlckName,NameOfSignal, "PASS", "::", self.ModuleName,
                     Rule_ID,'Signal Name and Port names are matched in BlockType:\"'+BlockType+'" of Name:\"'+BlckName+'" in System or SubSystem:\"'+Name+'".')                                        
            else:
                self.dataLoggerObj.logCondResult(BlckName,NameOfSignal,"FAIL", "::", self.ModuleName,
                     Rule_ID,'Signal Name and Port names are mis-matched in BlockType:\"'+BlockType+'" of Name:\"'+BlckName+'" in System or SubSystem:\"'+Name+'".')
        elif chekType == 'Match':
            if NameOfSignal != '<unknown>':		
                if NameOfSignal != '' or  len(NameOfSignal) != 0: 
                    self.dataLoggerObj.logCondResult("-","-", "PASS", "::", self.ModuleName,
                         Rule_ID,'Output signal Name of Block:\"'+BlckName+'" is labelled in System or SubSystem:\"'+Name+'"')                                        
                else:
                    self.dataLoggerObj.logCondResult("-","-","FAIL", "::", self.ModuleName,
                         Rule_ID,'Output signal Name of Block:\"'+BlckName+'" must be labelled with proper name in System or SubSystem:\"'+Name+'"')
            else:
                self.dataLoggerObj.logCondResult("-","-","FAIL", "::", self.ModuleName,
                     Rule_ID,'Output signal Name of Block:\"'+BlckName+'" must be labelled in System or SubSystem:\"'+Name+'"')
        elif chekType == 'Match2':
            if numOfSignalsinLine == noOfPortSignals:
                self.dataLoggerObj.logCondResult("-","-", "PASS", "::", self.ModuleName,
                     Rule_ID,'Output signal Names of Block:\"'+BlckName+'" are labelled in System or SubSystem:\"'+Name+'"')                                        
            else:
                self.dataLoggerObj.logCondResult("-","-","FAIL", "::", self.ModuleName,
                     Rule_ID,'Output signal Names of Block:\"'+BlckName+'" must be labelled with proper name in System or SubSystem:\"'+Name+'"')
            ...
            if resultType == 'Match':
                ...
                for IndxKey in range(0,len(self.BusSignals)):
                    ...
                    if len(self.BusCreatorOutputSignalName) == 0:
                       	self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                             Rule_ID,'Check(in root levels of this model) whether listed BusSignals are created by BusCreator or not.If not,FAIL this rule.')
                        break
                    else:
                        ...
                        if BusSignalExist == 0:
                  	        self.dataLoggerObj.logCondResult("-", "-", "MANUAL", "::", self.ModuleName,
                                 Rule_ID,'Check(in root levels of this model) whether BusSignal:\"'+self.BusSignals[IndxKey]['signalName']+'" of Block:\"'+self.BusSignals[IndxKey]['BlockName']+'" is created by BusCreator or not.If not,FAIL this rule.')
    ...
    def processAllRulesC(self):
        ...
        RULE_ID = ''
        #Configuration Settings Check, MISRA AC SLSF 003, MISRA AC SLSF 004, HISL_0013 A and HISL_0005 C
        self.checkConfigSettings_RCAPI()
        ...
        if MISRACheckbox == True:
            ...
            self.checkSystemPropValueInBlock_RCAPI(RULE_ID, Input['PropChkData3'], Input['UniqueKey'], [], [], 'NEGATIVE')            # RULE __________________________________________________________________________________
            ...
            Input = DataDictionary.RuleCheckerInput[RULE_ID]
            self.InputSignalshouldLabel_RCAPI(RULE_ID,Input['matchType'],Input['UniqueKey'])
            # RULE __________________________________________________________________________________
            ...
            Input = DataDictionary.RuleCheckerInput[RULE_ID]
            self.SignalLabelsShouldName(RULE_ID,Input['AllowedBlock'], Input['ListType'])
            # RULE __________________________________________________________________________________

            RULE_ID = 'MISRA AC SLSF 027 C'
            Input = DataDictionary.RuleCheckerInput[RULE_ID]
            self.InportLabelString_RCAPI(RULE_ID,Input['PropChkData'],Input['ListType'], Input['ResultMatchType'])
            # RULE __________________________________________________________________________________

            RULE_ID = 'MISRA AC SLSF 027 D'
            Input = DataDictionary.RuleCheckerInput[RULE_ID]
            self.OutportLabelString_RCAPI(RULE_ID,Input['PropChkData'],Input['ListType'], Input['ResultMatchType'])
            # RULE __________________________________________________________________________________
            ...
            Input = DataDictionary.RuleCheckerInput[RULE_ID]
            #This function checks only Bus creator            
            self.InputSignalshouldLabel_RCAPI(RULE_ID,Input['matchType'],Input['UniqueKey'])            
            # rest of the blocks(MUX,,SubSystem,Chart) are displaying for manual            
            self.checkTagNameAndSignalOrBusMustMatch_RCAPI(RULE_ID, Input['PropChkData'], Input['UniqueKey2'], Input['ResultMatchType'],Input['AllowedBlock'])
            # RULE __________________________________________________________________________________
        ...
        if RICARDOCheckbox == True:
            ...
            Input = DataDictionary.RuleCheckerInput[RULE_ID]
            self.InputSignalshouldLabel_RCAPI(RULE_ID,Input['matchType'],Input['UniqueKey'])
            self.InputSignalshouldLabel_RCAPI(RULE_ID,Input['matchType'],Input['UniqueKey2'])
            # RULE __________________________________________________________________________________
            if self.mdlName == self.SelectedModel:
                RULE_ID = 'RP_0059'
                Input = DataDictionary.RuleCheckerInput[RULE_ID]
