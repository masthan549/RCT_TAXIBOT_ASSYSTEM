...
class DataLogger(object):
    ...
    def __init__( self, fileName, RuleCheckObj ):
        self.__fileName = fileName
        csv.register_dialect('rulcheckres', delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        try:
            self.__csvAutoHandle = open(fileName+"_Auto.csv", 'wb')
            self.__csvManHandle = open(fileName+"_Man.csv", 'wb')
        except:
            RuleCheckObj.dispMsgBox('Please close all the csv files and retry. ', 'Error')
            RuleCheckObj.Close()
    ...
    def logCompResult(self, expval, actval, seperator, checkType, *arbargs):
        ...
        if PassFailStatus == "FAIL":
            ...
            self.__dispText = "MODEL      >>>  %30s \n\nRULE         >>>  %30s >>> %30s\n\nREMARKS  >>>  %s \n" %((arbargs[0].ljust(30)).upper(), arbargs[1].ljust(30),
                               RuleDetails.ljust(5), Remarks.ljust(30))
            self.__RuleCheckObj.styledTextCtrl1.AppendText(self.__dispText)
            self.__RuleCheckObj.styledTextCtrl1.AppendText('-'* 175 + "\n")

        else:
            Remarks = seperator.join(arbargs[2:-1])
        if len(Remarks) == 0:			
            Remarks = "Actual and Expected values are matched,hence this rule is PASS"        
